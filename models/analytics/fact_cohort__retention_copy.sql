WITH fact_sales__source AS (
  ...
  left join data-warehouse-course-391316.wide_world_importers_dwh.dim_customer as dim_customer
  using (customer_key)
)
...
, fact_cohort__retention AS (
  ...
  LEFT JOIN fact_sales__first_latest_order_month USING (customer_id)
)
...
, fact_cohort__retention_densed AS (
  SELECT  
    ...
    , COUNT(customer_id) AS active_user
